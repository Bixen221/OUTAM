...
export default function AdminDashboard() {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center text-gray-400">Vérification admin...</div>;
      ...
      <header className="bg-gray-900 text-white">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={async () => { await supabase.auth.signOut(); router.push('/'); }} className="text-sm text-gray-400 hover:text-red-400">Déconnexion</button>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-8">
          <div className="bg-white rounded-2xl p-5 border border-gray-100 text-center">
            <div className="text-3xl font-bold text-brand-500">{restaurants.length}</div>
            <div className="text-xs text-gray-400 mt-1">Restaurants</div>
          </div>
          <div className="bg-white rounded-2xl p-5 border border-gray-100 text-center">
            <div className="text-3xl font-bold text-brand-500">{dishes.length}</div>
            <div className="text-xs text-gray-400 mt-1">Plats</div>
          </div>
          <div className="bg-white rounded-2xl p-5 border border-gray-100 text-center">
            <div className="text-3xl font-bold text-green-500">{totalScans}</div>
            <div className="text-xs text-gray-400 mt-1">Scans total</div>
          </div>
          <div className="bg-white rounded-2xl p-5 border border-gray-100 text-center">
            <div className="text-3xl font-bold text-amber-500">{scansToday}</div>
            <div className="text-xs text-gray-400 mt-1">Scans aujourd&apos;hui</div>
          </div>
          <div className="bg-white rounded-2xl p-5 border border-gray-100 text-center">
            <div className="text-3xl font-bold text-purple-500">{restaurants.length ? Math.round(totalScans / restaurants.length) : 0}</div>
            <div className="text-xs text-gray-400 mt-1">Moy. scans/resto</div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          {filtered.length === 0 ? (
            <p className="text-center text-gray-400 py-12">Aucun restaurant trouvé.</p>
          ) : filtered.map((r) => {
            ...
            return (
              <div key={r.id} className="bg-white rounded-2xl p-5 border border-gray-100 hover:border-brand-200 transition-colors">
                <div className="flex items-center gap-4">
                  {r.logo_url ? <img src={r.logo_url} alt="" className="w-12 h-12 rounded-xl object-cover flex-shrink-0" /> : <div className="w-12 h-12 rounded-xl flex items-center justify-center text-lg font-bold flex-shrink-0" style={{ background: (r.theme_color||'#3300FF') + '15', color: r.theme_color||'#3300FF' }}>{r.name[0]?.toUpperCase()}</div>}
